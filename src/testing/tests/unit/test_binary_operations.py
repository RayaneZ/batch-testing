...
import pytest
from shtest_compiler.parser.shunting_yard import parse_validation_expression, BinaryOp, Atomic
...
class TestBinaryOperations:
    ...
    def test_simple_validation_wrapping(self):
        ...
        # Verify the content
        assert ast.left.value == "Le fichier /tmp/test.txt est présent", f"Left should be 'Le fichier /tmp/test.txt est présent', got {ast.left.value}"
        assert ast.right.value == "true", f"Right should be 'true', got {ast.right.value}"
    
    def test_compound_validation(self):
        """Test compound validation with multiple conditions."""
        # Test compound validation
        ast = parse_validation_expression("Le fichier /tmp/test.txt est présent et Le fichier /tmp/test.txt est vide")
        ...
        # Verify the content
        assert ast.left.value == "Le fichier /tmp/test.txt est présent", f"Left should be 'Le fichier /tmp/test.txt est présent', got {ast.left.value}"
        assert ast.right.value == "Le fichier /tmp/test.txt est vide", f"Right should be 'Le fichier /tmp/test.txt est vide', got {ast.right.value}"
    ...
    def test_simple_validation_shell_generation(self):
        ...
        assert "present" in shell_text or "présent" in shell_text, "Should contain 'present' or 'présent'"
    ...
    def test_compound_validation_shell_generation(self):
        ...
        assert "&&" in shell_text or "and" in shell_text, "Should contain logical operator"
    
    def test_complex_validation_expression(self):
        """Test complex validation expression with multiple operators."""
        # Test more complex expression
        ast = parse_validation_expression("Le fichier /tmp/test.txt est présent et Le fichier /tmp/test.txt est vide ou stdout contient 'test'")
        ...
        assert isinstance(ast.left, (Atomic, BinaryOp)), "Left should be Atomic or BinaryOp"
        assert isinstance(ast.right, (Atomic, BinaryOp)), "Right should be Atomic or BinaryOp"
    ...
    def test_validation_with_quotes(self):
        ...
        assert "'hello world'" in ast.left.value or "'hello world'" in ast.right.value, "Quoted content should be preserved"
    ...
    def test_single_word_validation(self):
        ...
        assert isinstance(ast.left, Atomic), "Left should be Atomic"
        assert isinstance(ast.right, Atomic), "Right should be Atomic"
        assert ast.left.value == "present", f"Left should be 'present', got {ast.left.value}"
        assert ast.right.value == "true", f"Right should be 'true', got {ast.right.value}" 