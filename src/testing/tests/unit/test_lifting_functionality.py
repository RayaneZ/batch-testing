...
class TestLiftingFunctionality:
    ...
    def test_lifting_inline_shell_code(self):
        ...
        inline_actions = [action for action in lifted_ast.steps[0].actions if isinstance(action, InlineShellCode)]
        assert len(inline_actions) > 0, "Should preserve InlineShellCode"
        
        # Verify code lines are present
        for action in inline_actions:
            assert hasattr(action, 'code_lines'), "InlineShellCode should have code_lines"
    ...
    def test_lifting_empty_actions(self):
        ...
        test_ast = ShtestFile(steps=[
            TestStep(name="Test Step", actions=[], lineno=1)
        ])
