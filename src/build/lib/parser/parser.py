...
class Parser:
    ...
    def __init__(self) -> None:
        ...
        self.arg_pattern = re.compile(r"(?:argument|paramètre)\s*:\s*(\w+)=?(\S*)", re.IGNORECASE)
        self.alias_rules = [
            (re.compile(r"variable\s+([^\s]+)\s+vaut\s+(.+)", re.IGNORECASE), lambda m: VarEquals(variable=m.group(1), expected=m.group(2))),
            (re.compile(r"stdout\s+contient\s+(.+)", re.IGNORECASE), lambda m: StdoutContains(expected=m.group(1))),
            (re.compile(r"stderr\s+contient\s+(.+)", re.IGNORECASE), lambda m: StderrContains(expected=m.group(1))),
        ]
