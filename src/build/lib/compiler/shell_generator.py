...
from parser.shunting_yard import SQLScriptExecution
            ...
            for action in actions["initialization"]:
                ...
                if scripts:
                    for script in scripts:
                        node = SQLScriptExecution(
                            script=script,
                            connection=args.get("SQL_CONN", "${SQL_CONN:-user/password@db}"),
                            driver=current_driver
                        )
                        script_lines, _ = node.accept(context.visitor)
