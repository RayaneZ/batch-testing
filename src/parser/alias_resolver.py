...
class AliasResolver:
    ...
    def _build_aliases(self) -> List[Tuple[re.Pattern, Callable[[re.Match], List[str]]]]:
        ...
        return [
            ...
            (re.compile(r"le fichier\s+est\s+cr[eé]é", re.I), lambda m: ["Le fichier est présent"]),
            (re.compile(r"le fichier\s+est\s+initialis[eé]", re.I), lambda m: ["Le fichier est présent"]),
            (re.compile(r"le dossier\s+est\s+(?:copi[ée])", re.I), lambda m: ["le dossier est copié"]),
            (re.compile(r"le dossier\s+est\s+pr[êe]t", re.I), lambda m: ["dossier créé"]),
            (re.compile(r"le dossier\s+est\s+cr[eé]é", re.I), lambda m: ["dossier créé"]),
