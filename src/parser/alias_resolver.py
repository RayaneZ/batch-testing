...
class AliasResolver:
    ...
    def _build_aliases(self) -> List[Tuple[re.Pattern, Callable[[re.Match], List[str]]]]:
        ...
        return [
            ...
            (re.compile(r"le script est affich[ée]", re.I), lambda m: ["le script est affiché"]),
            ...
            (re.compile(r"la date est modifi[ée]e", re.I), lambda m: ["date modifiée"]),
            (re.compile(r"le fichier est déplacé", re.I), lambda m: ["le fichier est déplacé"]),
            (re.compile(r"le dossier est déplacé", re.I), lambda m: ["le dossier est déplacé"]),
