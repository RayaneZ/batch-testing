    ...
    try:
        ...
        try:
            ...
            for i, step in enumerate(ast.steps):
                print(f"       Step {i}: '{step.name}' with {len(step.actions)} actions")
                for j, action in enumerate(step.actions):
                    print(f"         Action {j}: cmd='{action.command}', result='{action.result_expr}'")
                    print(f"           Raw line: '{action.raw_line}'")
            
        except Exception as e:
            print(f"   ConfigurableParser failed: {e}")
        
        # Test the legacy parser
        print("\n2. Testing Legacy Parser:")
        try:
            parser = Parser()
            ast = parser.parse(content, path=file_path, debug=True)
