...
class DirContainsCountValidation:
    ...
    def to_shell(self, varname="result", last_file_var=None):
        ...
        phrase, opposite = get_canonical_phrase_and_opposite("dir_contains_count", plugin_name="dir")
        if not dir_path or not self.count or not self.pattern:
            return [f"echo 'ERROR: Missing dir, count, or pattern for dir_contains_count validation'"]
        return [
            f"expected='{phrase}'",
