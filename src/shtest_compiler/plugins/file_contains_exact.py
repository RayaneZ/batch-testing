...
class FileContainsExactValidation:
    ...
    def to_shell(self, varname="result", last_file_var=None):
        ...
        phrase, opposite = get_canonical_phrase_and_opposite("file_contains_exact", plugin_name="file")
        if not file_path or not self.text:
            return [f"echo 'ERROR: Missing file or text for file_contains_exact validation'"]
        return [
            f"expected='{phrase}'",
