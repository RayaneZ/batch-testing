...
class ShtestParser:
    ...
    def parse(self, tokens: List[Token]) -> ShtestFile:
        ...
        for i, token in enumerate(tokens):
            if token.kind == "STEP":
                if self.debug:
                    debug_print(f"[DEBUG] Parser: Processing STEP token: value='{token.value}' at line {token.lineno}")
                ...
                if current_step is None and orphan_actions:
                    if self.debug:
                        debug_print(f"[DEBUG] Parser: Creating synthetic 'Orphan Actions' step with {len(orphan_actions)} actions")
                ...
                if self.debug:
                    debug_print(f"[DEBUG] Parser: Processing ACTION_ONLY token: value='{token.value}' at line {token.lineno}")
                
                if pending_action and current_step:
                    current_step.actions.append(pending_action)
                
                pending_action = Action(command=token.value, result_expr=None, result_ast=None, lineno=token.lineno)
                
                if current_step is None:
                    if self.debug:
                        debug_print(f"[DEBUG] Parser: Adding orphan action: {token.value}")
                    orphan_actions.append(pending_action)
                    pending_action = None
            
            elif token.kind == "RESULT_ONLY":
                if self.debug:
                    debug_print(f"[DEBUG] Parser: Processing RESULT_ONLY token: value='{token.value}' at line {token.lineno}")
                ...
                if pending_action:
                    ...
                    if current_step:
                        ...
                        if self.debug:
                            debug_print(f"[DEBUG] Parser: Adding orphan action with result: {pending_action.command} -> {token.value}")
                        orphan_actions.append(pending_action)
                    pending_action = None
                else:
                    action = Action(command=None, result_expr=token.value, result_ast=None, lineno=token.lineno)
                    if current_step:
                        current_step.actions.append(action)
                    elif current_step is None:
                        if self.debug:
                            debug_print(f"[DEBUG] Parser: Adding orphan result-only action: {token.value}")
                        orphan_actions.append(action)
            
            elif token.kind == "ACTION_RESULT":
                if self.debug:
                    debug_print(f"[DEBUG] Parser: Processing ACTION_RESULT token: value='{token.value}' at line {token.lineno}")
                ...
                if len(parts) == 2:
                    ...
                    action = Action(command=command, result_expr=result_expr, result_ast=None, lineno=token.lineno)
                else:
                    action = Action(command=token.value, result_expr=None, result_ast=None, lineno=token.lineno)
                
                if current_step:
                    current_step.actions.append(action)
                elif current_step is None:
                    if self.debug:
                        debug_print(f"[DEBUG] Parser: Adding orphan action-result: {action.command} -> {action.result_expr}")
                    orphan_actions.append(action)
        ...
        if pending_action:
            if current_step:
                ...
                if self.debug:
                    debug_print(f"[DEBUG] Parser: Adding final orphan action: {pending_action.command}")
                orphan_actions.append(pending_action)
        ...
        if orphan_actions and not shtest_file.steps:
            if self.debug:
                debug_print(f"[DEBUG] Parser: Creating final synthetic 'Orphan Actions' step with {len(orphan_actions)} actions")
            synthetic = TestStep(name="Orphan Actions", lineno=1)
