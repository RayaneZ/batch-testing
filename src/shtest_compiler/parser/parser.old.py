...
from ..config.rule_registry_hybrid import load_rules
from shtest_compiler.common_patterns import (
    ACTION_RESULT_RE,
    ACTION_ONLY_RE,
    RESULT_ONLY_RE,
...
class Parser:
    ...
    def __init__(self) -> None:
        self.rules: List[Rule] = [Rule(pattern=p, handler=h) for p, h in load_rules()]
    ...
    def parse(self, description: str) -> Dict[str, Any]:
        ...
        for line in description.splitlines():
            ...
            m = ACTION_RESULT_RE.match(stripped)
            if m:
                action_text = m[1].rstrip(" ;")
                result = m[2].rstrip('.;').strip()
                sub_parser = Parser()
                sub_actions = sub_parser.parse(action_text)
                self._merge_actions(sub_actions, actions)
                if result:
            ...
            for rule in self.rules:
                match = rule.pattern.search(stripped)
                if match:
                    rule.handler(actions, *match.groups())
