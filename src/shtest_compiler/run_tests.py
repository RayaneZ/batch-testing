...
def compile_e2e_tests(verbose=False):
    ...
    cmd = ["python", "-m", "shtest_compiler.run_all", "--input", "tests/e2e", "--output", "tests/integration"]
    if verbose:
        cmd.append("--verbose")
    ...
    for shtest_file in shtest_files:
        ...
        try:
            ...
            if hasattr(shtest_obj, 'steps') and isinstance(shtest_obj.steps, list):
                ...
                print(f"    {shtest_file.name} syntax error: Invalid ShtestFile structure")
                failed += 1
...
def main():
    parser = argparse.ArgumentParser(description="Test Runner for Batch Testing Framework")
    parser.add_argument("--unit", action="store_true", help="Run unit tests only")
    parser.add_argument("--e2e", action="store_true", help="Compile E2E tests only")
    parser.add_argument("--integration", action="store_true", help="Run integration tests only")
    parser.add_argument("--verify", action="store_true", help="Verify E2E test syntax only")
