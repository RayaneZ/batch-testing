...
with open(CONFIG_PATH, encoding="utf-8") as f:
    _PATTERNS = yaml.safe_load(f)  # _PATTERNS est un dictionnaire contenant les motifs regex nommés
...
ACTION_RESULT_RE = re.compile(_PATTERNS["action_result"], re.IGNORECASE)  # Pour les lignes avec action + résultat
ACTION_ONLY_RE = re.compile(_PATTERNS["action_only"], re.IGNORECASE)      # Pour les lignes contenant uniquement une action
RESULT_ONLY_RE = re.compile(_PATTERNS["result_only"], re.IGNORECASE)      # Pour les lignes contenant uniquement un résultat
COMMENT_RE = re.compile(_PATTERNS["comment"], re.IGNORECASE)             # Pour détecter les commentaires
