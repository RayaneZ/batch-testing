...
def get_handlers_by_category(category: str) -> List[str]:
    ...
    try:
        requirements_data = load_yaml_config("handler_requirements.yml")
        return [h for h, v in requirements_data.get("handlers", {}).items() if v.get("category") == category]
    except Exception:
        return []
...
def match_pattern_with_variables(pattern: str, text: str, case_sensitive: bool = False) -> Optional[Dict[str, str]]:
...
def canonize_action(action: str) -> Optional[tuple]:
    ...
    try:
        ...
        if debug_enabled:
            debug_print(f"[DEBUG] canonize_action: Loaded {len(action_patterns)} action patterns")
    ...
    if debug_enabled:
        debug_print(f"[DEBUG] canonize_action: Processing action '{action}' (normalized: '{action_lower}')")
    
    for pattern_entry in action_patterns:
        phrase = pattern_entry["phrase"]
        handler = pattern_entry["handler"]
        if debug_enabled:
            debug_print(f"[DEBUG] canonize_action: Checking pattern '{phrase}' with handler '{handler}'")
        ...
        if "{" in phrase and "}" in phrase:
            ...
            if debug_enabled:
                debug_print(f"[DEBUG] canonize_action: Testing regex pattern '{regex_pattern}'")
            match = re.match(regex_pattern, action, re.IGNORECASE)
            if match:
                if debug_enabled:
                    debug_print(f"[DEBUG] canonize_action: Found match with pattern '{phrase}'")
                return (
        ...
        if phrase.lower() == action_lower:
            if debug_enabled:
                debug_print(f"[DEBUG] canonize_action: Found exact match with pattern '{phrase}'")
            return (
                pattern_entry["phrase"],
                pattern_entry["handler"],
                pattern_entry
        ...
        for alias in pattern_entry.get("aliases", []):
            ...
            if "{" in alias and "}" in alias:
                ...
                if debug_enabled:
                    debug_print(f"[DEBUG] canonize_action: Testing alias regex '{regex_pattern}'")
                match = re.match(regex_pattern, action, re.IGNORECASE)
                if match:
                    if debug_enabled:
                        debug_print(f"[DEBUG] canonize_action: Found match with alias '{alias}'")
                    return (
                        pattern_entry["phrase"],
                        pattern_entry["handler"],
                        pattern_entry
            ...
            if alias.lower() == action_lower:
                if debug_enabled:
                    debug_print(f"[DEBUG] canonize_action: Found exact match with alias '{alias}'")
                return (
                    pattern_entry["phrase"],
                    pattern_entry["handler"],
                    pattern_entry
            ...
            if alias.startswith("^") and alias.endswith("$"):
                try:
                    if debug_enabled:
                        debug_print(f"[DEBUG] canonize_action: Testing regex alias '{alias}'")
                    if re.match(alias, action_lower, re.IGNORECASE):
                        if debug_enabled:
                            debug_print(f"[DEBUG] canonize_action: Found match with regex alias '{alias}'")
                        return (
                            pattern_entry["phrase"],
                            pattern_entry["handler"],
                            pattern_entry
...
def extract_variables_from_command(command: str, handler_name: str) -> Dict[str, str]:
    ...
    for pattern_config in patterns:
        ...
        if phrase:
            extracted = match_pattern_with_variables(phrase, command, case_sensitive=False)
            if extracted:
        ...
        for alias in pattern_config.get("aliases", []):
            if isinstance(alias, str):
                extracted = match_pattern_with_variables(alias, command, case_sensitive=False)
                if extracted:
                    variables.update(extracted)
                    break
...
def extract_context_from_action(action: str, handler_name: str) -> Dict[str, Any]:
    ...
    for pattern_config in patterns:
        phrase = pattern_config.get("phrase", "")
        if phrase and match_pattern_with_variables(phrase, action, case_sensitive=False):
        ...
        for alias in pattern_config.get("aliases", []):
            if isinstance(alias, str) and match_pattern_with_variables(alias, action, case_sensitive=False):
                context["patterns_matched"].append({
                    "type": "alias",
                    "pattern": alias,
                    "config": pattern_config
...
def validate_action_context(context: Dict[str, Any]) -> Tuple[bool, List[str]]:
    ...
    for var_name in required_vars:
        if var_name not in variables:
            errors.append(f"Handler {context.get('handler', 'unknown')} requires '{var_name}' variable")
    ...
    for var_name, var_value in variables.items():
        if var_name in validation_rules:
            ...
            if pattern and not re.match(pattern, var_value):
                errors.append(f"Variable '{var_name}' value '{var_value}' doesn't match pattern '{pattern}'")
        ...
        if phrase:
            ...
            for placeholder in extract_variables_from_pattern(phrase):
                if placeholder == "path":
                    ...
                    example = example.replace(f"{{{placeholder}}}", "SELECT * FROM table")
