...
def match(expected, last_file_var=None, scope="global"):
    ...
    for phrase, pattern_info in patterns.items():
        ...
        if normalized in [strip_accents(alias.lower()) for alias in aliases]:
            return _handle_pattern_match(
                handler_name, pattern_info, last_file_var, scope
            )
        ...
        for alias in aliases:
            if alias.startswith("^") and alias.endswith("$"):
                ...
                if re.match(regex_pattern, expected, re.IGNORECASE):
                    return _handle_pattern_match(
                        handler_name, pattern_info, last_file_var, scope, expected
                    )
...
def _handle_pattern_match(
    handler_name, pattern_info, last_file_var, scope, original_text=None
):
    ...
    try:
        ...
        plugin_module = __import__(
            f"shtest_compiler.plugins.{handler_name}", fromlist=[handler_name]
        )
        if hasattr(plugin_module, handler_name):
            ...
            if hasattr(plugin_instance, "to_shell"):
                ...
                return plugin_instance.to_shell(
                    varname="result", last_file_var=last_file_var
                )
