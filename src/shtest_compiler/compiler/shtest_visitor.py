...
def compile_shtest_file(ast: ShtestFile, verbose: bool = False) -> list[str]:
    lines = ['#!/bin/sh', 'set -e', '', 'run_cmd() {', '  local _stdout=$(mktemp)', '  local _stderr=$(mktemp)', '  /bin/sh -c "$1" >"$_stdout" 2>"$_stderr"', '  last_ret=$?', '  last_stdout=$(cat "$_stdout")', '  last_stderr=$(cat "$_stderr")', '  rm -f "$_stdout" "$_stderr"', '  if [ $last_ret -ne 0 ]; then', '    echo "STDERR: $last_stderr"', '  fi', '}', '', 'log_diff() {', '  expected="$1"', '  actual="$2"', '  if [ "$expected" != "$actual" ]; then', "    echo 'Différence détectée :'", '    echo "- Attendu : $expected"', '    echo "- Obtenu : $actual"', '  fi', '}', '']
