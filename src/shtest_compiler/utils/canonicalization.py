...
def get_canonical_phrase_and_opposite(handler, plugin_name=None):
    ...
    if plugin_name:
        plugin_path = os.path.join(os.path.dirname(__file__), f"../plugins/{plugin_name}/config/patterns_{plugin_name}.yml")
    ...
    builtin_path = os.path.join(os.path.dirname(__file__), "../config/patterns_validations.yml")
    with open(builtin_path, encoding="utf-8") as f:
        data = yaml.safe_load(f)
        for entry in data.get("validations", []):
            if entry.get("handler") == handler:
                phrase = entry.get("phrase")
                opposite = entry.get("opposite", {}).get("phrase") if entry.get("opposite") else None
                return phrase, opposite
    return None, None 
